...
import '../translations/bisayan_translation.dart';
...
class _SurahDetailPageState extends State<SurahDetailPage> {
  ...
  void initState() {
    super.initState();
    fetchSurah().then((_) {
      _loadTagalogTranslation();
      _loadBisayanTranslation();
    });
    _loadReciterPref();
  ...
  // 🔹 fetch Bisayan translation
  Future<void> _loadBisayanTranslation() async {
    final bisayaData = await BisayanTranslation.fetch(widget.surahNumber);
    if (bisayaData != null) {
      for (var i = 0; i < bisayaData["ayahs"].length; i++) {
        ayahs[i]["translation_bis"] =
        bisayaData["ayahs"][i]["translation_bis"];
      }
      setState(() {});
    }
  }
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  final shareText =
                                      "${ayah['text_ar']} "
                                      "\n\n${selectedLanguage == "maranao" ? (ayah['text_mn'] ?? "") : selectedLanguage == "tagalog" ? (ayah['translation_tl'] ?? "—") : (ayah['translation_bis'] ?? "—")} "
                                      "\n\n(${widget.surahNumber}:${ayah['ayah_number']})";
